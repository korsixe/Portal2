...
public class DataInitializer implements CommandLineRunner {
    ...
    private void ensureAdditionalRegularUserAnnouncements() throws IOException {
        List<TestAdSeed> seeds = List.of(
            new TestAdSeed("Алексей Иванов", "ivanov.aa@phystech.edu", "Электронный будильник Philips", "Компактный будильник с ярким дисплеем, работает без сбоев.", Category.HOME, "Освещение", Condition.USED, 1200, "Долгопрудный, ул. Первомайская, д. 18", "src/main/resources/photo/test_ads/alarm_clock.jpeg", List.of("Philips", "Черный", "Пластик", "Б/у хорошее")),
            new TestAdSeed("Алексей Иванов", "ivanov.aa@phystech.edu", "Bluetooth-колонка Sony", "Портативная колонка с чистым звуком и хорошим аккумулятором.", Category.ELECTRONICS, "Аксессуары", Condition.USED, 3500, "Долгопрудный, ул. Первомайская, д. 18", "src/main/resources/photo/test_ads/bluetooth_speaker.jpeg", List.of("Sony", "Синий", "Пластик", "Б/у отличное")),
            new TestAdSeed("Алексей Иванов", "ivanov.aa@phystech.edu", "Ковер для гостиной", "Мягкий домашний ковер, чистый и без повреждений.", Category.HOME, "Текстиль", Condition.USED, 4200, "Долгопрудный, ул. Первомайская, д. 18", "src/main/resources/photo/test_ads/carpet.jpeg", List.of("Бежевый", "Шерсть", "Б/у хорошее", "Современный")),

            new TestAdSeed("Мария Петрова", "petrova.mv@phystech.edu", "Компьютерная мышь Microsoft", "Удобная беспроводная мышь для учебы и работы.", Category.ELECTRONICS, "Компьютеры", Condition.USED, 1800, "Долгопрудный, ул. Бауманская, д. 7", "src/main/resources/photo/test_ads/computer_mouse.jpeg", List.of("Microsoft", "Серый", "Пластик", "Б/у отличное")),
            new TestAdSeed("Мария Петрова", "petrova.mv@phystech.edu", "Белый комод для комнаты", "Вместительный комод в хорошем состоянии, отлично подойдет в спальню.", Category.HOME, "Мебель", Condition.USED, 7800, "Долгопрудный, ул. Бауманская, д. 7", "src/main/resources/photo/test_ads/dresser.jpeg", List.of("Белый", "Дерево", "Б/у хорошее", "Минимализм")),
            new TestAdSeed("Мария Петрова", "petrova.mv@phystech.edu", "Проводные наушники Sony", "Легкие наушники для ежедневного использования, звучат чисто.", Category.ELECTRONICS, "Наушники", Condition.USED, 2300, "Долгопрудный, ул. Бауманская, д. 7", "src/main/resources/photo/test_ads/earphones.jpeg", List.of("Sony", "Черный", "Пластик", "Б/у хорошее")),

            new TestAdSeed("Даниил Сидоров", "sidorov.dk@phystech.edu", "Футбольный мяч", "Практически новый мяч для игры на улице и в зале.", Category.SPORTS, "Игровые виды", Condition.NEW, 1600, "Долгопрудный, Лихачевский проспект, д. 64", "src/main/resources/photo/test_ads/football.jpeg", List.of("Белый", "Резина", "Новое", "Спортивный")),
            new TestAdSeed("Даниил Сидоров", "sidorov.dk@phystech.edu", "iPhone в хорошем состоянии", "Рабочий смартфон Apple, аккумулятор держит уверенно.", Category.ELECTRONICS, "Смартфоны", Condition.USED, 29900, "Долгопрудный, Лихачевский проспект, д. 64", "src/main/resources/photo/test_ads/iphone.jpg", List.of("Apple", "Черный", "Стекло", "Б/у отличное")),
            new TestAdSeed("Даниил Сидоров", "sidorov.dk@phystech.edu", "Клавиатура для ПК", "Полноразмерная клавиатура для компьютера, клавиши работают мягко.", Category.ELECTRONICS, "Компьютеры", Condition.USED, 2500, "Долгопрудный, Лихачевский проспект, д. 64", "src/main/resources/photo/test_ads/keyboard.jpeg", List.of("Microsoft", "Черный", "Пластик", "Б/у хорошее")),

            new TestAdSeed("Елена Смирнова", "smirnova.ea@phystech.edu", "Настольная лампа Philips", "Яркая лампа для рабочего стола, отлично подходит для учебы.", Category.HOME, "Освещение", Condition.USED, 2100, "Долгопрудный, Дмитровское шоссе, д. 107", "src/main/resources/photo/test_ads/lamp.jpeg", List.of("Philips", "Белый", "Металл", "Б/у отличное")),
            new TestAdSeed("Елена Смирнова", "smirnova.ea@phystech.edu", "Ноутбук Lenovo для учебы", "Ноутбук в рабочем состоянии, браузер и офис тянет без проблем.", Category.ELECTRONICS, "Ноутбуки", Condition.USED, 25500, "Долгопрудный, Дмитровское шоссе, д. 107", "src/main/resources/photo/test_ads/laptop.jpg", List.of("Lenovo", "Серый", "Металл", "Б/у хорошее")),
            new TestAdSeed("Елена Смирнова", "smirnova.ea@phystech.edu", "Микроволновка LG", "Компактная микроволновая печь для общежития или съемной квартиры.", Category.HOME, "Бытовая техника", Condition.USED, 4900, "Долгопрудный, Дмитровское шоссе, д. 107", "src/main/resources/photo/test_ads/microwave.jpg", List.of("LG", "Белый", "Металл", "Б/у хорошее")),

            new TestAdSeed("Никита Козлов", "kozlov.np@phystech.edu", "Набор тарелок", "Керамические тарелки без сколов, использовались аккуратно.", Category.HOME, "Посуда", Condition.USED, 1700, "Долгопрудный, ул. Спортивная, д. 9", "src/main/resources/photo/test_ads/plate.jpg", List.of("Белый", "Керамика", "Б/у отличное", "Классический")),
            new TestAdSeed("Никита Козлов", "kozlov.np@phystech.edu", "Теплый свитер", "Шерстяной свитер на зиму, размер M, приятный к телу.", Category.CLOTHING, "Мужская одежда", Condition.USED, 2200, "Долгопрудный, ул. Спортивная, д. 9", "src/main/resources/photo/test_ads/sweater.jpeg", List.of("Серый", "Шерсть", "M", "Зима")),
            new TestAdSeed("Никита Козлов", "kozlov.np@phystech.edu", "Электрический чайник Bosch", "Быстро кипятит воду, корпус без трещин и запаха.", Category.HOME, "Бытовая техника", Condition.USED, 2600, "Долгопрудный, ул. Спортивная, д. 9", "src/main/resources/photo/test_ads/teapot.jpg", List.of("Bosch", "Серебристый", "Металл", "Б/у хорошее"))
        );
